...
| 提案 | 優先度 | 理由 |
|------|--------|------|
| `RARITY_THRESHOLDS` の閾値を実測分散値域に合わせ引き上げ（伝説:12000 / 英雄:8000 / 希少:4000 / 高級:1500）。現行閾値（伝説:4000）は一般的な自然画像の分散値域と乖離しており、大多数が「伝説」に判定される | 高 | バグ修正 |
| `analyzeRarity` 内で `variance` を `Math.min(variance, 10000)` でキャップし5段階に均等マッピングする正規化方式に変更（閾値調整と択一） | 中 | バグ予防 |
| 生成完了オーバーレイに分散値を表示する `DEBUG_RARITY` フラグ定数（`false`）を追加し閾値調整を容易にする | 低 | 使い勝手向上 |
...
| 空腹度が0のペットが庭に存在する場合、庭ナビアイコンに⚠️バッジを表示 |
| ペット生成時の種類出現率調整（閾値を実測edgeDensity値域に合わせ修正） |
| ペット生成時に40%の確率でランダム種類補正を追加（全10種の出現を保証） | 中 | UX改善 |
