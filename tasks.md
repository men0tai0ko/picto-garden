...
| 2 | 訓練ログに累計勝利数・連勝数を小計行として表示（セッション内のみ） | 中 | 使い勝手向上 |
| 3 | 訓練ペット選択後の敵属性表示に相性マーク（有利🔥/不利💧/等倍—）をインライン追記 | 低 | 使い勝手向上 |
| 4 | ケージ編集モードで「庭に出す」押下時、庭スロット残数を編集モードUI内にインライン表示 | 低 | 使い勝手向上 |
| 5 | 図鑑画面に解放済み種数カウンター（例：「3/10 解放済み」）をヘッダーに追加 | 低 | UX改善 |
