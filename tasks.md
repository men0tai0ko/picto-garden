...
| 4 | T5 報酬ループ | T4 | **実装完了** |
| 4 | T6 図鑑 | T1 | **実装完了** |
| 5 | T7 スキル・属性相性・レア度 | T4 | **実装完了** |
| 1 | T8 繁殖 | T1 | **実装完了** |
| 2 | T9 見た目変化（進化） | T3 | **実装完了** |
| 3 | T10 ペット名称・削除 | T2 | **実装完了** |
...
| # | 提案 | 優先度 | 理由 |
|---|------|--------|------|
| 1 | ペット生成時の種類出現率調整（閾値を実測edgeDensity値域に合わせ修正） | | |
| 2 | ペット生成時に40%の確率でランダム種類補正を追加（全10種の出現を保証） | 中 | UX改善 |
| 3 | 訓練ログに累計勝利数・連勝数を小計行として表示（セッション内のみ） | 中 | 使い勝手向上 |
| 4 | ケージカードの世代バッジ（`N世`）に世代数に応じたカラーを付与（1世:銀・2世以上:金） | 低 | UX改善 |
| 5 | `startHungerTimer` 初回発火前に `renderStatusBar()` を1回呼び出し、起動直後の通貨表示ズレを防止 | 低 | バグ予防 |
| 6 | 訓練ペット選択後の敵属性表示に相性マーク（有利🔥/不利💧/等倍—）をインライン追記 | 低 | 使い勝手向上 |
| 7 | ケージ編集モードで「庭に出す」押下時、庭スロット残数を編集モードUI内にインライン表示 | 低 | 使い勝手向上 |
| 8 | 図鑑画面に解放済み種数カウンター（例：「3/10 解放済み」）をヘッダーに追加 | 低 | UX改善 |
