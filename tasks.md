...
| 訓練画面でMP=0かつスキル未発動の場合にその旨をログへ1行追記 |
| 庭パネル「野に放つ」導線をパネルに追加 |
| 繁殖結果overlay（`showBreedResultOverlay`）に子ペットのレア度を表示 |
| `showEvictDialog`（追い出しoverlay）のペット行にHP・満腹度バーを追加 |
| 庭パネル「餌をあげる」ボタンを通貨不足時に disabled 化してテキストを「通貨不足」に変更 |
